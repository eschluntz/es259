function [ tool_pos, T] = puma_fk2(joints, fast)
%puma_fk2 returns the position of the tool frame relative to the base frame
...
% getting DH params
a = [0,upper_arm,0,0,0,0,0];
alpha = [-pi/2, 0, pi/2, -pi/2,0, pi/2, 0];
d = [shoulder_height,0,-ds,upper_arm,0,0, de];
theta = [joints(1),joints(2), joints(3)+pi/2,...
    0,joints(4), joints(5) - pi/2, joints(6)];
...
A7 = rotz(theta(7)) * transz(d(7)) * transx(a(7)) * rotx(alpha(7));
