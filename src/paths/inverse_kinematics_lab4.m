function [theta1, theta2, theta3, theta4, theta5] = inverse_kinematics_lab4(x_tool, y_tool, z_tool)
% inverse kinematics for use with obstacle avoidance
...
oc = [(x_tool - 3*cos(theta1));(y_tool - 3*sin(theta1));(z_tool+8)];
...
for i = 1:4
    if (solution(i,2) > 45*pi/180) || (solution(i,2) < 0*pi/180) || (solution(i,3) > 30*pi/180) || (solution(i,3) < -90*pi/180) || (solution(i,4) < -45*pi/180) || (solution(i,4) > 45*pi/180) || (solution(i,5) < -75*pi/180) || (solution(i,5) >75*pi/180)
        norm(i) = inf;
    else
